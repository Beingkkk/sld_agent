...
> **当前状态**：项目处于设计落地与打包验证阶段。`Document/` 下已锁定宪法、spec 及各模块 plan；`SourceCode/data/` 已放置 JSON 注册表、知识库、预转换的 Sample GeoJSON 与本地字体；`SourceCode/core` 已实现 SLD 树、Transformer、ValidationEngine、FilterTransformer 并包含测试；`SourceCode/frontend` 已实现 Vue 3 三栏 UI、树编辑、JSON 驱动属性面板、OpenLayers 地图预览、代码/XML 显示与 AI 解释面板；`SourceCode/backend` 已实现 WebSocket Agent、LLM 客户端、知识库注入；`SourceCode/electron` 已实现无边框窗口、IPC、后端子进程启动与生产期资源路径处理。root `package.json` 与 workspaces 已启用，可运行 `npm install`、`npm run typecheck`、`npm test`、`npm run build`、`npm run dist`。
...
# 独立启动前端（Vite dev server，端口 5173）
npm run dev:frontend

# 独立启动后端（WebSocket Agent，端口 18765）
npm run dev:backend

# Electron 开发模式（构建 electron 后同时启动 backend + frontend + electron）
...
# 打包 Electron 应用（调用 electron-builder，产物在 ./dist）
npm run dist
```
...
两个脚本都会执行 `npm ci`、清理构建产物、构建所有 workspaces、调用 `electron-builder`。产物输出到根目录 `dist/`（已在 `.gitignore` 中，不提交）。
...
## 7. CodeGraph MCP（结构查询优先）

本项目配置了 CodeGraph MCP 服务器（`codegraph_*` 工具）。它基于 tree-sitter 解析整个代码库，提供符号、调用关系、影响面等结构化查询，适合回答"哪里定义了 X"、"谁调用了 Y"、"修改 Z 会 break 什么"等问题。

优先使用 codegraph 做**结构性**探索，而非 grep/read 循环：

| 问题 | 工具 |
| --- | --- |
| 哪里定义了 X / 找符号 | `codegraph_search` |
| 谁调用了 Y | `codegraph_callers` |
| Y 调用了什么 | `codegraph_callees` |
| X 如何到达 Y | `codegraph_trace` |
| 修改 Z 的影响面 | `codegraph_impact` |
| 看多个相关符号源码 | `codegraph_explore` |
| 目录下有什么文件 | `codegraph_files` |

注意：文件写入后索引有约 500ms 的 debounce 延迟，同 turn 内不要立即重新查询刚修改的文件。

---

## 8. 关键决策记录

### 8.1 为什么用 GeoStyler 但不直接用 GeoStyler React UI？

GeoStyler 主包是 React + Ant Design，而 SLDAgent 前端是 Vue 3。复用报告结论：只复用 parser 与数据模型，UI 组件参考其设计后自行实现。

### 8.2 为什么后端用 Node.js/TypeScript？
...
### 8.9 Sample 数据为什么不放在 `public/sample/`？
...
### 8.10 OpenLayers 预览的样式函数 workaround
...
### 8.11 Electron 生产包的关键配置

- **`asar: false`**：Node ESM 无法从 asar 虚拟路径加载模块，因此禁用 asar，让 `backend/dist`、`frontend/dist`、`node_modules` 以真实文件形式存在于 `resources/app/`。
- **backend `package.json` 复制**：backend 是 ESM（`"type": "module"`），构建脚本会把 `package.json` 复制到 `dist/package.json`，确保运行时 Node 能识别 ESM。
- **backend 运行时依赖内聚**：`ajv`、`chalk`、`ws` 已加入 `SourceCode/electron/package.json` 的 `dependencies`，确保 electron-builder 把它们打包进产物。
- **生产期路径**：Electron main 通过 `app.getAppPath()` 定位 `frontend/dist/index.html`，通过 `process.resourcesPath` 定位 `data/` 和 `config/`，并通过环境变量 `SLDAGENT_DATA_PATH` / `SLDAGENT_CONFIG_PATH` 传给后端子进程。
- **相对资源路径**：前端 Vite 配置 `base: './'`，确保 `file://` 协议下静态资源路径正确；`@data` 别名指向 `SourceCode/data/`，sample GeoJSON 通过 `import ... ?url` 引用。
...
- 前端通过 Vite 的 `@data` 别名引用 `SourceCode/data/` 下的资源，并通过 `assetsInclude: ['**/*.geojson']` 让 GeoJSON 作为资源处理；生产环境下这些路径会被打包到 `frontend/dist/assets/`。
